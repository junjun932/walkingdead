...
//async &await 는 같이 다닌다 .  try , catch 같이 다님 .
async function fetchComments() {
  ...
  try {
    ...
    //배열에서 탁쟝 부븐 만 잘라내는 method  ( 0~10) -> 0~9 까지
    //slice method ; arr.slice([begin[, end]])
    ...
    slicedData.forEach((comment) => {
      ...
      //    comment의 body의 길이가 40 이상일 때    /commentbody.length >=40
      //    0번째부터 39번째 문자를 복사하고 맨 뒤에 "..."을 붙인 문자열을       ...  body.slice(0,40)
      //    slicedBody라는 변수에 할당해주세요.

      //    if  comment body.length >=40 ,  comment body.slice(0,40) + "...";
